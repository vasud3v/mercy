...
class RoleManager(commands.Cog, name="Role Management"):
    ...
    def __init__(self, bot):
        self.bot = bot
        # Use shared MongoDB connection from bot
        self.client = bot.mongo_client
        if not self.client:
            logging.warning("RoleManager: No shared MongoDB connection available")
            self.db = None
        else:
            self.db = self.client.role_manager
            logging.info("RoleManager: Using shared MongoDB connection")
        
    async def cog_load(self):
        """Set up the database when cog loads"""
    ...
    def cog_unload(self):
        """Cleanup when cog is unloaded - don't close shared MongoDB connection"""
        logging.info("RoleManager: Cog unloaded")
